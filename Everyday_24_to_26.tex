...
\begin{frame}
    ...
    \begin{analysis}
        \begin{itemize}[<+-| alert@+>]
            \item 我们可以设\(n\)是一个\(k\)位自然数，且首位数字是\(4\)，则\(n = 4 \times 10^k + m\)，其中\(k\)和\(m\)都是自然数，\(k \ge 2\)，\(m < 10^k\)。
            \item 于是，变换之后的数字为\(n' = 10 \times m + 4\)。
            \item 根据条件，我们有\(n = 4n'\)
            \item 接下来，分析\(m\)的性质即可。
        \end{itemize}
    \end{analysis}
...
\begin{frame}
    % \frametitle{每日一题（24）}
    \begin{solution}
        设 \(n = 4 \times 10^k + m\)，\(n' = 10 \times m + 4\)，其中\(k\)和\(m\)都是自然数，\(k \ge 2\)，\(m < 10^k\)，\\
        则 \(n = 4n'\)，即 \(4 \times 10^k + m = 4 \times \left(10 \times m + 4\right)\)。\pause
        ...
        经验证，\(10^2 \equiv 9 \pmod{13}\)，\(10^3 \equiv 12 \pmod{13}\)，\(10^4 \equiv 3 \pmod{13}\)，\(10^5 \equiv 4 \pmod{13}\)，所以 \(k\) 最小是 \(5\)。\pause
